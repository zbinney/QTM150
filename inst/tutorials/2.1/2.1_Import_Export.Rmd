...
### Working Directories in Quarto

Quarto notebooks (`.Rmd` files) work differently than R scripts.

In Quarto files, the working directory is *automatically* set to be the location of the Quarto file. As long as that's what you want, there's no need to set it explicitly.

If that's *not* what you want you have to change the working directory, but differently than how we discussed before. The most common way would be in the `setup` chunk, usually at the beginning of the Quarto document just below the YAML header. You would use code like this inside that chunk: `knitr::opts_knit$set(root.dir = '[FILE PATH FOR WORKING DIRECTORY]')`.
