...
```{r, out.width = "100%", fig.cap = "*Like these wise fellows!*", echo = FALSE}
...
```{r, out.width = "100%", fig.cap = "*How to set your working directory to the script location manually.*", echo = FALSE}
...
RMarkdown notebooks (`.Rmd` files) work differently than R scripts.

In RMarkdown files, the working directory is *automatically* set to be the location of the RMarkdown file. As long as that's what you want, there's no need to set it explicitly.

If that's *not* what you want you have to change the working directory, but differently than how we discussed before. The most common way would be in the `setup` chunk, usually at the beginning of the RMarkdown document just below the YAML header. You would use code like this inside that chunk: `knitr::opts_knit$set(root.dir = '[FILE PATH FOR WORKING DIRECTORY]')`.

For more on manipulating working directories in RMarkdown, see [chapter 16.6 of Yihui Xie's incredible resource the *R Markdown Cookbook*](https://bookdown.org/yihui/rmarkdown-cookbook/working-directory.html).
...
```{r, out.width = "100%", fig.cap = "*A successful import.*", echo = FALSE}
