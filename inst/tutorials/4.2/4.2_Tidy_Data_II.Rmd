---
title: "Tutorial 4.2 - Tidy Data II"
...
In this one we will focus on how to **reshape** data by switching between long and wide format. This is a very common task because different analyses may be more easily conducted and different questions more easily answered in either long or wide format.
...
```{r pivot_longer1}
table4a <- table4a |> 
  mutate(yr_2001 = c(1700, 51000, 213000)) |>  # Adding a fourth column so going to long format
                                                # will actually result in fewer columns (from 4 in wide to 3 in long format)
  rename(yr_2000 = `2000`, # Renaming columns so later code will work in learnr
         yr_1999 = `1999`)
table4a
...
<mark>Challenge:</mark> What if I hadn't added the third year column `yr_2001` above? What would have happened to the dimensions of our wide and long data frames? Do you notice anything interesting about that? 

<mark>Challenge:</mark> Duplicate and modify the code above to turn `table4b` from wide to long. It should require several minor modifications. (Recall `table4b` was one of our tables of disease data from Tutorial 4.1. It is replicated in the code chunk below.)

```{r chal_table4b, exercise = TRUE}
# Change `table4b` from wide to long format
table4b 
...
Let's discuss briefly what I mean by using `select()` syntax for the `cols =` argument. Briefly, anything you can do with `select()` you can do here. So in `table4a` I wanted to select the columns `yr_1999`, `yr_2000`, and `yr_2001`. Here are two ways I could have done that:

```{r pivot_longer_select}
# Change `table4a` from wide to long format
table4a |> 
  pivot_longer(cols = yr_1999:yr_2001,
               names_to = "year",
               values_to = "cases")
