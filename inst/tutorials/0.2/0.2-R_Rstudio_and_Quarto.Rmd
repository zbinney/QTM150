---
title: "Tutorial 0.2 - Intro to R, Rstudio and Quarto"
...
description: "Intro to R, Rstudio and Quarto"
---
...
This document - and all the other future ones in this course - are interactive tutorials, built using Quarto and the `learnr` **package** in R (we'll talk more about what Quarto and "package" mean in a moment). You have the opportunity to actually run R code in this document itself in "Exercises." There are also sometimes "Questions" for you to answer and test your knowledge.
...
WARNING: if you try to do this while running a Tutorial, you may notice it won't seem to run. That's because you can't run both at the same time in the same RStudio "session." RStudio has to dedicate all its attention in one session to watching the interactive tutorial. But there's a super easy fix to do both at the same time: in the top menu go to Session \> New Session. This will open up a second RStudio window. You should be able to run any code you want in that second window while the first one keeps an eye on your tutorial. (Note you can run code in the tutorial without a second session, this is only for if you want to follow along and code in your own script as you go.)
...
-   Quarto. It's what I used to produce this cool interactive document. It's a way of using a language called "Markdown" alongside R code, in the RStudio interface, to create pretty reports, presentations, and other documents that interweave your code, graphics, and explanatory text. It's also how you will submit your homework.

    -   You can just write R code with written comments in an R "script" (a file of code that ends `.R`). But Quarto creates a `.qmd` file that lets you write and format (e.g. bolding and italicizing and bullets) text to explain your commented code and also show its output, and then share that as a website (HTML output), PDF document, or many other options.

Some analogies may help. Let's go to our first interactive question of the course!

```{r r-analogies, echo=FALSE}
  question("Which of the following do you think might make for a good analogy for R, RStudio, and Quarto?",
    answer("Cars", message = "Sorry, try again!"),
    answer("Cooking", message = "Yum, I wish! Try again."),
    answer("Movies", correct = TRUE, message = "That'll work! Keep reading below."),
    answer("Shrek", message = "While Shrek and R both have layers, much like an onion, this is incorrect- try again."),
    type = "learnr_radio",
    allow_retry = TRUE)
```
...
Quarto is an alternative way of presenting your movie (code). It allows you to add more depth to your movie (code) by adding in extra information like a narrative discussion of what you did and why, or the output of the code. The movie (code) is still there, but now it has a bunch of extra features like what might come on a DVD if any of you still watch those. It allows an even richer movie-watching experience.
...
```{r arithmetic}
#This is static code and output in Quarto.  This green text is a "Comment" that is not run as code.
...
Now, let's take a look at how often each country voted "Yes" on a resolution in each year. We'll pick a few countries of interest, then visualize the results:
...
Next, let's create a visualization that displays how the voting record of the United States changed over time on a variety of issues, and compare it to another country. The other country we'll display is India for starters.
...
It will be helpful to start with a map of what happens when you open a .R or .qmd file (some code) in RStudio. Here's a tour of the basic areas:
...
4.  A miscellaneous window. There are a few important things in this pane:
    ...
    ii) **Plots** - when you create a plot by running code in a **script**, this is where it shows up (it will show up in-line in the Quarto document if you produce it there).
...
The **Console** is on the lower left hand corner of RStudio (though it will be the entire left hand side of the screen when you open RStudio without a **script** or Quarto file, such as the first time you turn it on).
...
-   Cons: If you share the script all you see is your code and comments, not all the pretty formatting and output you can include in an Quarto document.

-   When to use it: if you don't care about sharing your code with non-analysts, people who can't run it themselves, and you don't want to fold in output and graphs into a pretty PDF or HTML document that looks more like a report than a code script.
...
### Option 3: Quarto Document (Most Common for This Class)

The R installation instructions you read through last week were the product of Quarto. Quarto files weave together explanatory text (beyond code comments), code, and the output from that code to make a readable report you can share with anyone, including non-programmers! You can create web pages (HTML), Word documents, PDFs, whole blogs, and even books. It's an incredibly flexible way to write code and present your findings and is much prettier and complete than a script.

-   How to use it: You write text and code in a `.qmd` file just like you would in a script, except you have a lot more options. You then **knit** this `.qmd` file into your desired output, such as a web page (HTML) or PDF. More below.
...
-   Cons: More complex than just writing a script.
...
<mark>Challenge:</mark> Try opening the one for Quarto now!
...
Most often these add one or more new **functions** to those R knew before.

R **functions** are just like mathematical functions. Consider f(x) = 2x. The function, f(), takes one or more inputs (or in computer terms, **arguments**, in this case "x"), does something to those inputs, and returns an output. Note functions can, and often do, take more than one argument.
...
-   **Load** packages. You have to do this every time you turn R on - when you leave or restart R it goes back to its innate functions only. This is code you have to run over and over, so it should be placed in a **script** or Quarto document rather than the console.
...
I want you to use the `p_load()` function from the `pacman` package. This function has two big benefits: it can load as many packages as you want, separated by commas (while `library()` can only do one at a time); and it automatically installs any packages in that list it detects you're missing. Magical! Here's how you use it:
...
Oops! It doesn't know the function `beep()`. So let's install and load the `beepr` package by running this code in your Console: `pacman::p_load("beepr")`
...
I can't show you the full capabilities of Quarto in an interactive tutorial. Instead, I want you to open up your own file. To do that while viewing this tutorial you need to:

1.  Create a new **Session** (open a new RStudio window). Click `Session` \> `New Session`.

2.  Create a new Quarto file. Click `File`, then `New file`, then `Quarto Document`. When a window pops up, just click "OK" for now.

You should now have something that looks like this in your script area:

```{r, echo = FALSE, out.width = "100%", fig.cap = "*New Quarto document.*"}
knitr::include_graphics("images/RStudio_Quarto.png")
```

Let's walk through the document, top to bottom. But first...

<mark>Challenge:</mark> **Knit** your first Quarto document! Knit the default raw `.qmd` file into an HTML document by hitting the `Render` button on the top ribbon. It should pop up in a new window. Now you can start to compare parts of the raw Quarto with what it prints in a finished document!

-   The **You Ain't a Markup Language (YAML) Header**. This is always the first thing in an Quarto document, and it'll look something like this:
...
```
    ...
    -   title: the title on the final rendered document. Note what it looks like in your rendered document.

    -   format: the format of the finished document when you click on the "Render" button. It'll look like a web page. To render a PDF instead, change this line to `pdf`. Note this isn't explicitly printed anywhere in the finished document; it tells the finished document what format to take.
    
    -   It's useful to add two other fields as a general practice. 
    author: the author on the final knitted document. Note what it looks like in your rendered document.
    date: hopefully self-explanatory at this point. Note what it looks like in your rendered document.

    -   There are plenty of other things you can add to your header. SEE the Quarto cheat sheet for some examples, but most of them probably look like gibberish to you right now. And that's OK!

<mark>Challenge:</mark> Change the title and add an author name and date to your HTML document. Re-render it to see the change reflected.


-   A **Heading** followed by some text. Compare how it's written in the raw Quarto to the finished document. We'll discuss this more below.
...
-   A **Code Chunk** that looks like this:
    ...
    -   Notice how the chunk appears in the raw Quarto versus how it's rendered and printed in the final document. You see the code *and the output*. The output is identical to what you'd see in the **console** if you just ran the code there or in a script (try it if you don't believe me!).

Below this is just another header, text, and code chunk that demonstrates how you can run chunks with certain options, which we will discuss later.
...
-   Click on the "+c" button in the ribbon menu then `R`, or

-   Press `Ctrl + Alt + i` on a PC or `Command + Option + i` on a Mac.

To run code chunks, you can either:

-   Run a *full single* chunk by clicking on the green triangle at the top-right of the chunk.

-   Run *part of a chunk* with `Ctrl/Command + Enter` just as you would in a normal script.

-   Run *all chunks up to this point in the document* by clicking on triangle with the line below it.

In any case, R's output appears right under the chunk in the raw Quarto `.qmd` file.

To run *all chunks* in a document you simply **Render** the document. As you've already seen, this exports the text, code chunks, and output into a pretty and portable format like HTML or PDF.

<mark>Challenge:</mark> Try adding a new code chunk to the default Quarto document that adds your favorite five numbers and returns the sum. Add a new heading and some narrative text to explain what the chunk is doing, as well. Re-knit the document to see the results.
...
1.  Quarto should evaluate this block as `r` code (rather than, say, Python code, which it can also handle).
...
3.  Chunk options, which always appear after the language, the optional name, and a comma `,`. These serve as instructions for the knitting process around specific chunks of code. The most common options include:
    ...
    i)  `{r [CHUNK-NAME], include=FALSE}`. This causes Quarto not to print the code *or* any output.

    ii) `{r [CHUNK-NAME], echo=FALSE}`. This causes Quarto not to print the code, but it will print any output.

    iii) `{r [CHUNK-NAME], eval=FALSE}`. This causes Quarto to print the code, but *not* any output.

You can list multiple options for a chunk, all separated by commas. The Quarto cheat sheet has a list of more chunk options.

Note that when you open a new Quarto document it shows you how to add options by using a line such as `#| echo: false` in the code chunk itself, but I prefer the method I've outlined above. Either is fine, though!
...
There are many ways to format text in Quarto. You can also use LaTeX to expand this, but that's beyond the scope of this course so don't worry about it. Here are some of the more common formats you might want to have in a knitted document:
...
### My Quarto Formatting Expectations: A Manifesto

Part of learning how to use R as a data analyst is learning how to cleanly and professionally communicate your findings, not just how to code. To that end a substantial portion of your grade is going to depend not just on getting the correct solution, but in presenting it in an easy-to-digest way.

On Canvas, if you click on "Modules", you'll see a section titled "Quarto Organization Example." My advice is to download the HW1 Example file and edit it for your first assignment. Then when another assignment comes around open the previous finished assignment, save it to a different name, delete *most* things except for one example of each main component (e.g. a heading, a code chunk), and then create the next assignment in it. This should start you from a good template each time. But the key things I'm looking for are:

-   **Headings**. Use them to orient and walk readers through your document. Note how I use them in my Tutorials and try to mimic that.

    -   They should be brief (usually one line) but clear so the reader knows what to expect in a given section (e.g. Loading Data and Packages, Analysis of Flight Departure Delays).

    -   Start big with a title, then go smaller and smaller for sub-sections as needed. If you get to the 4th level (`####`) or smaller I recommend bolding so it stands out from the normal text.

-   All code should be in **code chunks** (i.e. when writing it should be in a box with a gray background). It should not be simply written into the document outside of a chunk except in very specific cases such as pulling a single number into a paragraph of written text that will auto-update (none of which I'll be asking you to do in this course).

-   Code chunks should be interweaved with explanatory text before and/or after.

    -   I favor the following pattern:

        1)  If needed (i.e. not self-explanatory or made clear elsewhere like in a heading), 1-2 sentences of text before a chunk (depending on its length) explaining what the code is doing.

        2)  The code chunk itself.

        3)  If needed (i.e. it produces output like a plot or table that isn't self-explanatory), a brief analysis of the output after the chunk.

    Repeat as you move through the document.

    Use your judgement as to whether 1/3 are needed in any particular case, but keep in mind you shouldn't assume your audience can quickly figure out what your code is doing by looking at the code itself and, if the output was produced to answer a specific question, highlight that answer in an analysis of the output.

-   Package names (e.g. `nycflights13`), data frame names (e.g. `flights`), variable names (e.g. `tailnum`), and function names (e.g. `mean()`), *if written in text outside of a code chunk*, should be formatted with backticks \`\`

    -   Please note backticks \`\` are different from apostrophes ''. Backticks are on the tilde key next to the 1-key on a standard English QWERTY keyboard.

-   Do not **bold** or *italicize* entire paragraphs or blocks of text. Most text should simply be typed in without formatting. Use bolding, italics, and other formatting for key words and phrases you want to emphasize.

If you're confused about what I mean by any of this, refer to the HW1 Example file. I've designed it to reflect the level of detail and effort I want to see on your assignments.
