...
I can't explain it any better than Hadley Wickham in *R for Data Science (2nd ed.)*, so why try? Go now and read [chapter 3.1.3 and 3.4](https://r4ds.hadley.nz/data-transform.html#dplyr-basics). They're pretty brief, I promise! Return to this tutorial once you have.

NOTE: We are learning the newer "native" pipe (`|>`) that was added to base R versions from 2023 and onward. Previously this class taught, and you will often see in all but the most recent code examples, the `%>%` version of the pipe that is part of the `tidyverse`. These are, for the purposes of this class, essentially interchangeable. There are some minor differences I'm sure you'll pick up if you continue your programming career, but I'm not going to cover them here.

I'll just also remind you here that there's a useful shortcut for typing the pipe in RStudio: **CTRL/COMMAND +SHIFT + M**. But make sure you make the change as outlined in Chapter 3.4 to get `|>` rather than `%>%`.
...
Let's go through a few more examples of how useful the pipe can be in day-to-day data analysis.
...
### Other Ways to Use `summarize()` and `group_by()`

Means and counts are probably the most common reasons you'll use `summarize()` and `group_by()`, but there are lots of other things you can do.
