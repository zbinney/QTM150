---
title: "Tutorial 10.1 - Data Cleaning I - General Intro"
...
We also need to install `tidyimpute` to give us some options for replacing missing values. That we have to install separately from Github, the code for which I've included below but commented out. **Please be sure to run it on your own machines.** 

*Some students have had difficulty installing `tidyimpute` - if you are one of them, don't worry about it - simply remove `tidyimpute` from out `p_load()` call, and comment out lines 693-4 and 724-5. Then proceed with the Tutorial without needing to actually run `tidyimpute`.*
...
pacman::p_load(tidyverse, janitor, palmerpenguins, tidyimpute, fivethirtyeight)
...
You may be tempted to use `drop_na()` - which drops observations with *any* missing values for the specified variables (or for *any* variable if you don't specify any). It's the "simplest" way to deal with missing data, and leaves you with a nice, clean, complete data frame.

But simple != good.

And I urge caution here. I don't like simply throwing out observations without a very, very good reason. In addition to dumping the missing values, you're also dumping a lot of useful, known, non-missing data on other variables. It's too arbitrary and sacrifices information. Not a good practice to get into. 
...
That said, some analyses - like regression models - require this implicitly. If you ask to regress a company's stock price on its P/E ratio and dividends paid in the last year, then you can't include any companies (or company-quarters) for which that information is missing. If you haven't run a regression before this may not make sense - just move on.
...
The last method was based on the `tidyimpute` package, which despite its name is not part of the `tidyverse`. You should have installed it separately on your own machines.

You should be able to parse the basics of what all that code did, but to help orient you to the general structure of an `impute()` call:

```{r impute_str, eval = FALSE}
...
Note if you want to impute different values for different columns you'll need to use a separate `impute()` call.
...
* You could, instead, simply impute all missings with the overall mean value of a variable (or the most common category if it's categorical). This has a similar flaw as static imputation - you could assign a very wrong value to many observations! - but it may be better if there's no particular reason for data to be missing. That is, if stroke victims were just missing `fim` data completely at random, perhaps just filling in those missings with the mean `fim` value more or less makes sense.
    ...
    Here's an easy way to do that using `tidyimpute`:
    
```{r impute_mean, exercise = TRUE}

# Impute mean for missing FIM using tidyimpute::impute()
stroke %>%
  impute_mean(fim)
...
In the next few Tutorials we'll cover more specifically how to do cleaning and manipulation of strings (using `stringr`), factors (using `forcats`, which we've also covered a bit back in Tutorial 4.2), and dates (with `lubridate`).

```{r recode_as_factor2, include = FALSE, eval = FALSE}
...
stroke_clean <- stroke %>% 
  ...
  mutate(MARITAL = factor(MARITAL, # Change marital from its current type (integer) into a factor
         ...
         NEWFAC_NUM = factor(NEWFAC_NUM,
                             levels = c(1:10),
                             labels = c("New Jersey", "New York 1", "Iowa", "California", "Illinois",
                                        "Texas 1", "DC", "New York 2", "Texas 2", "Kentucky")),
         FEMALE = factor(FEMALE,
                         levels = c(0:1),
                         labels = c("Male", "Female")),
         DIAG_IG = factor(DIAG_IG,
                          levels = c(1:5),
                          labels = c("Left-body Weakness", "Right-body Weakness", "Bilateral Weakness",
                                     "No Weakness", "Other Stroke")),
         Age.Category = factor(Age.Category,
                               levels = c(1:3),
                               labels = c("60 and under", "61-80 years", "81+ years"))) %>% 
  # BLOCK B
  clean_names() %>% 
...
str_view(words, "ing$", match = TRUE)
str_view(words, ".*amy.*", match = TRUE)
