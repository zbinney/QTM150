...
Factors are great for a few reasons, but one major one is they help with sorting. You can specify the order of values of a categorical variable when that's important - think months. Months aren't in alphabetical order, so you want a way to set the order (Jan, Feb, Mar...).

Sometimes when visualizing data you will also want categories plotted in a specific order that makes the plot more aesthetically appealing. Let's try to plot the number of hours of TV watched per day by religion:
...
If you're creating a bar plot to count the number of appearances of various factor levels you can also reorder factors by their frequency using `fct_infreq()`. Consider the two plots below:
