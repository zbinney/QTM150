...
```{r setup}
pacman::p_load(tidyverse, palmerpenguins, knitr, skimr)
```
...
quiz(
  question("What are the dimensions of the `penguins` dataset?",
      answer("344 rows x 8 columns", correct = TRUE, message = "That's right! This dataset has 344 observations (i.e. rows, or penguins) and 8 variables (i.e. columns) of data about each one."),
      answer("8 rows x 344 columns"),
      answer("8 rows x 8 columns"),
...
Try running the below code in your own script. (NOTE: the output looks slightly different than what `Quarto` prints below.)
...
* The **dimensions** are 344 rows x 8 columns...unless you used the `head()` or `tail()` function, in which case R truncates the dataset behind the scenes and now treats `penguins` as having only 6 rows.
...
`skimr`'s `skim()` function mixes some of the elements from both `str()` and `summary()`, renders beautifully in Quarto, and provides a small histogram next to the numerical summaries of continuous variables, allowing you to immediately see its distribution visually (normal, skewed, bimodal, etc.).
...
* Check out [this *excellent* guide](https://nickch-k.github.io/DataCommSlides/Lecture_04_Common_R_Problems.html#things-breaking) from Nick Huntington-Klein at Seattle University. It lists some common error messages, what the typical sources of those are, and how to fix them. This is a GREAT first step when you're troubleshooting.
...
I *love* raincloud plots because they show a ton of information, mitigate the disadvantages of both histograms and density plots, and yet don't overwhelm the viewer. The code below (and given above) is for a quick and dirty version of one of these plots, but there are also [much more detailed (and complex) examples](https://rpubs.com/rana2hin/raincloud). Let's break it down line-by-line: 
...
* How to open and explore datasets already loaded in R
    ...
    + Native tools, `skimr` package
    
* Data visualization using `ggplot()`

    + Geoms/Chart types for single variables:
    
        - Histograms using `geom_histogram()` and frequency polygons using `geom_freqpoly()`
