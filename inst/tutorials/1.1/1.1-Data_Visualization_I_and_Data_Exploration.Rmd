...
#### The `skimr` and `summarytools` Packages
...
skim(penguins) 
```
Another option is `summarytools::dfSummary()`. Let's see how it works:

```{r dfSummary, out.width = "100%"}
print(dfSummary(penguins, graph.magnif = 0.75), 
      method = 'render')
```
The biggest benefit of `dfSummary()` is you get clearer histograms *and* horizontal frequency bar charts for categorical variables, which can be easier to scan than the mere numbers you get from `skim()`.

Note in order to get this to show properly in the Tutorial you may need to hover over the table and use your arrow keys to scroll right and left. I could not figure out how to set the proper width for this interactive Tutorial, but it should work fine in the normal RMarkdown documents you'll produce.

You don't have to use `skimr::skim()` or `summarytools::dfSummary()`. There are tons of other options out there. Find one you like and work with it!

But even with something like `skimr` or `summarytools`...just looking at tables of numbers and data types, it can be hard to really get a good feel for your data. What if we tried creating some pretty graphs and looking at things visually? That often tells us a lot more.
