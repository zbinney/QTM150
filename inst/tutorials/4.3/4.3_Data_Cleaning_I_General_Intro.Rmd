...
* If it's actually supposed to be a missing observation, re-code it to `NA`
...
```{r char_to_num2, exercise = TRUE, exercise.setup = "char_to_num"}
...
```{r char_to_num3, exercise = TRUE, exercise.setup = "char_to_num"}
...
#### DON'T DROP ALL MISSING DATA!!!

A tempting common sense solution when you find missing data is to just drop all your observations with missing data so you have only clean and complete data left. This is sometimes referred to as **complete case analysis**. **IT IS A VERY BAD IDEA! YOU WILL NOT DO IT IN THIS CLASS!**
...
You could also induce massive bias if, for example, a subpopulation refused to answer (or couldn't answer, because it wasn't applicable, like pregnancy history for people who can't get pregnant) 1-2 questions on a survey - and you just threw out all their data entirely! Or if a question was not asked in the 2024 version of a survey and you just threw out all data from 2024.

Plus, this could substantially reduce your sample size.

Instead, just note which variables have a lot of missing data, look for patterns, and think about whether you still want to use that variable in your analysis at all. Assuming you do, let the functions in R drop those observations as needed for specific analyses where they are missing key data. For example, if you run a regression model R's `lm()` (for linear model) function will only run the regression on observations with the outcome and all predictors. If you are creating a visualization, `ggplot()` will, for example, at the time it runs, exclude observations for a scatterplot that lack data on the X- or Y-coordinate. You don't need to manually do this ahead of time and risk excluding those same observations from *other* analyses where they have everything you need.

In summary: exclude observations with missing values on an analysis-by-analysis basis and as late as possible, right when you're doing the analysis itself.

As a corollary, if you Google around for solutions to missing data you will see a LOT of people using `na.omit()` or `drop_na()`. Avoid these solutions like the plague because they do what I've just explained above. They will drop observations with *any* missing values for *any* variable.
...
```{r fill2, exercise = TRUE, exercise.setup = "fill1"}
...
```{r complete, exercise = TRUE, exercise.setup = "create_data"}
